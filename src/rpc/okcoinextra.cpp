 ...
 * @return
 */
UniValue addmonitor(const JSONRPCRequest& request)
{
    ...
    for(unsigned long i = 0; i < request.params.size(); i++)
    {
        UniValue array(UniValue::VARR);
        UniValue param = request.params[i];
...
UniValue delmonitor(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() == 0)
        throw runtime_error(
...
UniValue ismonitor(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() == 0)
        throw runtime_error(
...
UniValue ackmonitor(const JSONRPCRequest& request)
{
    ...
    UniValue param = request.params[0];
...
UniValue ackblock(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            "ackblock \"requestId\"\n"
...
UniValue resynctx(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            "resynctx \"txId\"\n"
    ...
    if(confirmed)
    {
        ...
        paddressMonitor->SyncConnectBlockRPC(&block, pblockindex, *tx);
    }
...
/*
    ...
    if(request.params.size() > 1)
    {
       for(unsigned long i = 1; i < request.params.size(); i++)
       {
            UniValue array(UniValue::VARR);
            UniValue param = request.params[i];
            if(param.isStr() && !CBitcoinAddress().SetString(param.get_str()))
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    { "okcoinextra",         "addmonitor",            &addmonitor,      true, {"bitcoinaddress"} },
    { "okcoinextra",         "delmonitor",            &delmonitor,      true, {"bitcoinaddress"} },
    { "okcoinextra",         "ismonitor",             &ismonitor,       true, {"bitcoinaddress"} },
    { "okcoinextra",         "ackmonitor",            &ackmonitor,      true, {"requestId"} },
    { "okcoinextra",         "resynctx",              &resynctx,        true, {"txid"} },
    { "okcoinextra",         "ackblock",              &ackblock,        true, {"requestId"} },
    { "okcoinextra",         "rescan",                &rescan,          true, {"block","address"} },
};

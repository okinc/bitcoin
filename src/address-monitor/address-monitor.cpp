                                     ...
                                     int posInBlock)
{
...

	{
		...
		for (unordered_map<int, uint160>::const_iterator it = monitorMap.begin(); it != monitorMap.end(); it++)
		{
            ...
            ret.push_back(buildValue(tx.GetHash(), tx, it->first, NULL, addressTo,
...
void AddressMonitor::SyncConnectBlockRPC(const CBlock *pblock,
                                      CBlockIndex* pindex,
                                      ...
                                      const boost::unordered_map<uint160, std::string> &addresses)
{
...

	{
        ...
        BOOST_FOREACH(const CTransactionRef ptx, pblock->vtx)
		{
            unordered_map<int, uint160> monitorMap = GetMonitoredAddresses(*ptx, addresses);

			for (unordered_map<int, uint160>::const_iterator it = monitorMap.begin(); it != monitorMap.end(); it++)
			{
				string addressTo = addressMap[it->second];

                ret.push_back(buildValue(ptx->GetHash(), *ptx, it->first, pblock, addressTo,
						blockHash, nHeight, now, status));
			}
		}
	}
...
void AddressMonitor::SyncDisconnectBlock(const CBlock *pblock)
{
...

	{
        ...
        BOOST_FOREACH(const CTransactionRef ptx, pblock->vtx)
		{
            unordered_map<int, uint160> monitorMap = GetMonitoredAddresses(*ptx);
